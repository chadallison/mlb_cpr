...
team_margins |>
  ...
  coord_cartesian(xlim = c(vmarg_mean - 1.5, vmarg_mean + 2),
                  ylim = c(dmarg_mean - 1.75, dmarg_mean + 1.75)) +
...
team_colors |>
  mutate(col = 1,
         team = fct_rev(factor(team))) |>
  ggplot(aes(team, col)) +
  geom_col(aes(fill = team), show.legend = F) +
  coord_flip() +
  scale_fill_manual(values = rev(team_color_codes))
```
