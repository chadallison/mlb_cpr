...
vis_df = end_games |>
  mutate(win_score = ifelse(home_score > away_score, home_score, away_score),
         lose_score = ifelse(home_score > away_team, away_score, home_score),
         win_team = ifelse(home_score > away_score, home_team, away_team),
         lose_team = ifelse(home_score > away_score, away_team, home_team),
  ...
  ungroup() |>
  arrange(desc(n)) |>
  mutate(team_lab = paste0(win_team, " (", month, ")")) |>
  rename(wins = n) |>
  slice_max(wins, n = 25) |>
  head(25)

vis_fills = vis_df |>
  left_join(team_colors, by = c("win_team" = "team")) |>
  pull(hex)

vis_df |>
  ggplot(aes(reorder(team_lab, wins), wins)) +
  geom_col(aes(fill = team_lab), show.legend = F) +
  geom_text(aes(label = wins), size = 3, hjust = -0.25) +
  coord_flip() +
  scale_fill_manual(values = vis_fills) +
  labs(x = NULL, y = "Number of Wins", title = "Most Wins in a Single Month, 2023 Season") +
  theme(axis.text.x = element_blank())
```
