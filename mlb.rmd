...
loop_pks = loop_pks[!loop_pks %in% player_hrs_df$game_pk]

# player_hrs_df = data.frame()

for (i in 1:length(loop_pks)) {
  if (length(loop_pks) > 0) {
    ...
    if (nrow(loop_data) == 0) {
      loop_data = data.frame(game_pk = loop_pks[i], hitter_name = "Shohei Ohtani", hitter_team = "Los Angeles Angels", hrs = 0)
      player_hrs_df = rbind(player_hrs_df, loop_data)
    } else if (nrow(loop_data) > 0) {
      player_hrs_df = rbind(player_hrs_df, loop_data)
    }
  }
}

player_hrs_df = player_hrs_df |>
  distinct(game_pk, hitter_name, hitter_team, hrs)

write_csv(player_hrs_df, "player_hrs.csv")

top_hr_players = player_hrs_df |>
  group_by(hitter_name) |>
  summarise(hrs = sum(hrs)) |>
  slice_max(hrs, n = 5) |>
  pull(hitter_name)

# vis_colors = player_hrs_df |>
#   filter(hitter_name %in% top_hr_players) |>
#   inner_join(all_game_pks |>
#   filter(date <= Sys.Date() - 1 & !game_pk %in% bad_pks) |>
#   select(game_pk, date), by = "game_pk") |>
#   group_by(hitter_name) |>
#   slice_max(date, n = 1) |>
#   ungroup() |>
#   mutate(hitter_name = factor(hitter_name, levels = top_hr_players)) |>
#   arrange(hitter_name) |>
#   inner_join(team_colors, by = c("hitter_team" = "team")) |>
#   pull(hex)

player_labels = player_hrs_df |>
  group_by(hitter_name) |>
  summarise(hrs = sum(hrs)) |>
  slice_max(hrs, n = 5) |>
  transmute(hitter_name, label = paste0(hitter_name, " (", hrs, ")"))
...
player_hrs_df |>
  ...
  filter(date <= Sys.Date() - 1 & !game_pk %in% bad_pks) |>
  select(game_pk, date), by = "game_pk") |>
  group_by(hitter_name, date) |>
  summarise(hrs = sum(hrs),
            .groups = "drop") |>
  group_by(hitter_name) |>
  mutate(cum_hrs = cumsum(hrs),
         hitter_name = factor(hitter_name, levels = top_hr_players)) |>
  group_by(hitter_name) |>
  mutate(max_date = max(date)) |>
  ungroup() |>
  group_by(hitter_name) |>
  mutate(total_hrs = max(cum_hrs)) |>
  ungroup() |>
  inner_join(player_labels, by = "hitter_name") |>
  mutate(label = factor(label, levels = player_labels$label)) |>
  ggplot(aes(date, cum_hrs)) +
  geom_line(aes(col = label), linewidth = 2) +
  labs(x = NULL, y = "Home Runs", title = "2023 Home Run Leaders", col = NULL) +
  scale_y_continuous(breaks = seq(0, 100, by = 5)) +
